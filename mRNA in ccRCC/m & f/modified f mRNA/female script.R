...
pheno <- read_delim("D:/ccRCC project/mRNA in ccRCC/gdc_sample_sheet.2019-11-20.tsv", 
                    "\t", escape_double = FALSE, trim_ws = TRUE)
...
getwd()
setwd("D:/ccRCC project/mRNA in ccRCC/m & f/modified f mRNA")
...
f_exp = exp.sub_f[,keepSamples]
f_exp =apply (f_exp, 2,as.integer)
rownames(f_exp)=rownames(normal.exp_f)


f_pheno = pheno.sub_f[keepSamples,]

dim(f_exp)
dim(f_pheno)
dds_f = DESeqDataSetFromMatrix( countData = f_exp , colData = f_pheno , design = ~ Sample.Type)
...
res.degs_f_adj01=res.df_f[res.df_f$padj< 0.01 & abs(res.df_f$log2FoldChange)>log2(2),]
...
write.table(res.degs_f, file = "res-degs_f.txt", col.names = T , row.names = T , quote = F)
...
res.degs_f[res.degs_m$log2FoldChange<0,]$regulation =-1
res.degs_f[res.degs_m$log2FoldChange>0,]$regulation =1
res.degs2_f= cbind(rownames(res.degs_f), res.degs_f$regulation)
write.table(res.degs2_f, file("res_reg_f.txt"), row.names = F,
...
write.xlsx(exp.degs_f, file = "exp_degs_f.xlsx")
...
ntd_f=normTransform(dds_m)
exp.sub.norm_f= assay(ntd_m)
